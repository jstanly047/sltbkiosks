...
public class KafkaConfig {
    ...
    private String checkoutReplyTopic;
    ...
    @Bean
    public KafkaMessageListenerContainer<String, CheckIn> checkinReplyContainer(ConsumerFactory<String, CheckIn> cf) {
        ContainerProperties containerProperties = new ContainerProperties(checkinReplyTopic);
        return new KafkaMessageListenerContainer<>(cf, containerProperties);
    }

    @Bean
    public ConsumerFactory<String, CheckIn> checkInConsumerFactory() {
        return new DefaultKafkaConsumerFactory<>(consumerConfigs(),new StringDeserializer(),new JsonDeserializer<>(CheckIn.class));
    }

    @Bean
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<String, CheckIn>> checkInKafkaListenerContainerFactory() {
    ...
    //CheckIn
    @Bean
    public ProducerFactory<String, CheckOut> checkOutProducerFactory() {
        return new DefaultKafkaProducerFactory<>(producerConfigs());
    }
    ...
    @Bean
    public ReplyingKafkaTemplate<String, CheckOut, CheckOut> checkOutReplyKafkaTemplate(ProducerFactory<String, CheckOut> pf, KafkaMessageListenerContainer<String, CheckOut> container){
        return new ReplyingKafkaTemplate<>(pf, container);

    }

    @Bean
    public KafkaMessageListenerContainer<String, CheckOut> checkOutReplyContainer(ConsumerFactory<String, CheckOut> cf) {
        ContainerProperties containerProperties = new ContainerProperties(checkoutReplyTopic);
        return new KafkaMessageListenerContainer<>(cf, containerProperties);
    }

    @Bean
    public ConsumerFactory<String, CheckOut> checkOutConsumerFactory() {
        return new DefaultKafkaConsumerFactory<>(consumerConfigs(),new StringDeserializer(),new JsonDeserializer<>(CheckOut.class));
    }

    @Bean
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<String, CheckOut>> checkOutKafkaListenerContainerFactory() {
    ...
    @Bean
    public KafkaTemplate<String, Transaction> transactionOutKafkaTemplate() {
